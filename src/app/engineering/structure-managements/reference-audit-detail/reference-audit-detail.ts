// app/engineering/structure-managements/reference-audit-detail/reference-audit-detail.ts
...
@Component({
  selector: 'app-reference-audit-detail',
  standalone: true,
  imports: [CommonModule, FormsModule, GlobalFilterPipe],
  templateUrl: './reference-audit-detail.html',
  styleUrl: './reference-audit-detail.scss'
})
export class ReferenceAuditDetail implements OnInit {
  ...
  isAuditing = false;
  result: ReferenceAuditDetailResult | null = null;
  ...
  audit(): void {
    ...
    const payload: ReferenceAuditDetailPayload = {
    ...
    this.productStructureService
      .auditReferenceDetail(payload)
      .pipe(finalize(() => (this.isAuditing = false)))
      .subscribe({
        next: (res) => {
          if (!res?.ok) {
            this.toastr.error('No se pudo ejecutar el detalle de auditoría.', 'Error');
            return;
          }
          this.result = res.msg;
        },
